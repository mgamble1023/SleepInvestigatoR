...
o	Not the same as wake epoch count which counts every non-consecutive instance of wake no matter if it proceeds or follows another instance of wake. 
o	Result is the same as if you set wake cutoff to 1 for wake bout count.
...
o	E.g., If a bout is user defined as 15 epochs (60 sec if scored at 4 sec) then it will only count bouts of wake less than 15 epochs.
o	If wake cutoff is set to 1 then it will count only bouts equal to cutoff value.
