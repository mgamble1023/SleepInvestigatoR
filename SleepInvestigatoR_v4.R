...
FilePath <- "F:/SleepInvestigatoR_Project/Formatted Scored Files/BL" #e.g.,'C:/Users/John Smith/Desktop/Sleep Scores'
#FilePath <- "C:/Users/gamblm/Desktop/test"
...
SleepInvestigatoR <- function(FileNames, file.type = "csv", epoch.size = 4, max.hours = NULL, byBlocks = NULL, byTotal = TRUE, 
                              score.checker = TRUE, id.factor = TRUE, group.factor = TRUE, normalization = NULL, time.stamp = F, 
  ...
  if (byTotal == TRUE) {
    # byTotal Matrix
    mat.byTotal <- matrix(NA_real_, ncol = 97, nrow = length(FileNames))
...
for (FileName in FileNames) {
  ...
  # Load in scores
  if(time.stamp == TRUE & scores.column == 1){
  ...
  } else if(time.stamp == TRUE & scores.column == 2){
  ...
  for (i in seq_along(rle_scores$values)) {
    # Check if 0s are surrounded by 4s
    if (rle_scores$values[i] == 0 &&
        i > 1 && i < length(rle_scores$values) &&
        rle_scores$values[i - 1] == 4 &&
        rle_scores$values[i + 1] == 4) {
  ...
  }#convert 0s sandwiched by 4 to 4s -- this converts all wake less than wake bout cutoff between two instances of sleep to sleep epochs
  ...
  if(!is.null(byBlocks)){
    ...
    modified.scores.list <- split(modified.scores, ceiling(seq_along(modified.scores)/floor(length(modified.scores)/byBlocks)))
  }
  ...
  if(value.check == T){
    ...
    stop(paste("Scores in", FileName, "are not in the required 1,2,3 format."))
  }
  
  
  # Check for Wake-REM Transitions
  if(score.checker == TRUE){
 ...
 if(byTotal == TRUE){
  ...
  mat.byTotal[counter:counter,'Wake.Percent'] <- as.numeric(Wake.Percent)*100
  
  #NREM Percentage byTotal
  NREM.Percent <- mean(scores == 2)
  
  mat.byTotal[counter:counter,'NREMS.Percent'] <- as.numeric(NREM.Percent)*100
  
  #REM Percentage byTotal
  REM.Percent <- mean(scores == 3)
  
  mat.byTotal[counter:counter,'REMS.Percent'] <- as.numeric(REM.Percent)*100
  ...
  if(!is.null(byBlocks)) {
    ...
    if(time.stamp == TRUE){
      ...
      seqs <- seq_along(Dates)
      Dates.collapsed <- tapply(Dates,rep(seqs,each=((max.hours*3600/epoch.size)/byBlocks))[seqs],FUN=mean) #match main sheet
      Dates.collapsed <- as.Date(Dates.collapsed, format = '%y/%m/%d')
      
      mat.byBlocks[x.range1[counter]:x.range2[counter],'Dates'] <- Dates.collapsed
      ...
      seqs <- seq_along(hms.elapsed)
      hms.collapsed <- tapply(hms.elapsed,rep(seqs,each=((max.hours*3600/epoch.size)/byBlocks))[seqs],FUN=max)
      
      
      mat.byBlocks[x.range1[counter]:x.range2[counter],'Days.Elapsed.from.Recording.Start'] <- hms.collapsed
  ...
  NREM.Offset.Lengths[lengths(NREM.Offset.Lengths) == 0] <- NA
  ...
  NREM.Offset[lengths(NREM.Offset) == 0] <- NA
  ...
  REM.Offset.Lengths[lengths(REM.Offset.Lengths) == 0] <- NA
  ...
  REM.Offset[lengths(REM.Offset) == 0] <- NA
  ...
  Sleep.Offset.Lengths[lengths(Sleep.Offset.Lengths) == 0] <- NA
  ...
  Sleep.Offset[lengths(Sleep.Offset) == 0] <- NA
  ...
  mat.byBlocks[x.range1[counter]:x.range2[counter],'Wake.Percent'] <- as.numeric(unlist(Wake.Percent[1:byBlocks]))*100
  
  #NREM Percentage byBlocks
  NREM.Percent <- sapply(scores.list, function(x) mean(x == 2))
  
  mat.byBlocks[x.range1[counter]:x.range2[counter],'NREMS.Percent'] <- as.numeric(unlist(NREM.Percent[1:byBlocks]))*100
  
  #REM Percentage byBlocks
  REM.Percent <- sapply(scores.list, function(x) mean(x == 3))
  
  mat.byBlocks[x.range1[counter]:x.range2[counter],'REMS.Percent'] <- as.numeric(unlist(REM.Percent[1:byBlocks]))*100
  ...
  #Sleep Cycle Count byBlocks
 
  Sleep.Cycle.Count <- sapply(modified.scores.list, function(x) {  
    r <- rle(x)
    sum(r$values == 4 & r$lengths >= Cycle.cutoff)
  })
  
  mat.byBlocks[x.range1[counter]:x.range2[counter],'Sleep.Cycle.Count'] <- as.integer(unlist(Sleep.Cycle.Count[1:byBlocks]))
  ...
  Wake.Bout.Duration <- sapply(scores.list, function(x) {
    r <- rle(x)
    y <- data.frame(lengths = r$lengths, values = r$values)
    y <- y[y$lengths >= Wake.cutoff & y$values == 1,]
    mean(y$lengths)
  })
